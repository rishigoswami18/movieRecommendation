...
TMDB_CACHE: Dict[Tuple[str, Tuple[Tuple[str, str], ...]], Dict[str, Any]] = {}
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Safe TMDB GET:
    - reuses one AsyncClient
    - memoizes identical TMDB calls in-process
    - Network errors -> 502
    ...
    cache_key = (path, tuple(sorted((str(k), str(v)) for k, v in q.items())))
    if cache_key in TMDB_CACHE:
        return TMDB_CACHE[cache_key]

    if tmdb_client is None:
        raise HTTPException(status_code=500, detail="TMDB client not initialized")
