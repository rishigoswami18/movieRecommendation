...
# =============================
with st.sidebar:
    st.markdown("## ðŸŽ¬ Menu")
    if st.button("ðŸ  Home"):
        goto_home()

    st.markdown("---")
    st.markdown("### ðŸ  Home Feed (only home)")
...
st.markdown(
    "<div class='small-muted'>Type keyword â†’ dropdown suggestions + matching results â†’ open â†’ details + recommendations</div>",
    unsafe_allow_html=True,
)
st.divider()
...
if st.session_state.view == "home":
    ...
    # HOME FEED MODE
    st.markdown(f"### ðŸ  Home â€” {home_category.replace('_',' ').title()}")
...
# ==========================================================
    ...
    if title:
        ...
        if not err2 and bundle:
            st.markdown("#### ðŸ”Ž Similar Movies (TF-IDF)")
            poster_grid(
            ...
            st.markdown("#### ðŸŽ­ More Like This (Genre)")
            poster_grid(
